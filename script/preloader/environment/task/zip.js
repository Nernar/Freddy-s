Object.defineProperty(Environment.Task, "Pack", {
	...
	if (json != null && typeof json == "object") {
		...
		if (json.hasOwnProperty("must_explore")) {
			if (typeof json.must_explore == "string") {
				...
				log("Pack task must_explore must be string or boolean");
			}
		}
	} else if (typeof json == "string") {
		this.path = new Environment.Formation(this.getEnvironment(), json);
	} else {
		log("Pack task must be object or string")
	}
};
...
Environment.Task.Pack.prototype.process = function() {
	...
	packZip(this.getPath(), who, this.getOutput());
	return true;
};
